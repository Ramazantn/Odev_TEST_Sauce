...
class Sauce_Test:
    ...
    def mission_1(self, username, password):
        ...
        self.driver.save_screenshot((f"{self.folderPath}/mission_1-{username}-{password}.png"))
        assert hatalimesaj.text=="Epic sadface: Password is required"
    
    @pytest.mark.parametrize("username, password", [("standard_user","")])
    def mission_2(self, username, password):
        self.waitForElementVisible((By.ID, "user-name"))
        ...
        giris=self.driver.find_element(By.ID, "login-button")
        giris.click() 
        self.driver.save_screenshot(f"{self.folderPath}/mission_2-{username}-{password}.png")
    ...
    def mission_3(self,username,password):
        self.waitForElementVisible((By.ID, "user-name"))
        kullaniciAd=self.driver.find_element(By.ID, "user-name")
        sifre=self.driver.find_element(By.ID, "password")
        giris=self.driver.find_element(By.ID, "login-button")
        ...
        self.driver.save_screenshot(f"{self.folderPath}/mission_3-{username}-{password}.png")
        assert hataligiris.text == "Epic sadface: Sorry, this user has been locked out."
    
    @pytest.mark.parametrize("username,password",[("","")])   
    def mission_4(self,username,password):
        kullaniciAd=self.driver.find_element(By.ID, "user-name")
        kullaniciAd.send_keys(username)
        sifre=self.driver.find_element(By.ID, "password")
        sifre.send_keys(password)
        ...
        self.driver.save_screenshot(f"{self.folderPath}/mission_4.png")
        assert hata1==hata2
    
    @pytest.mark.parametrize("username,password", [("standard_user", "secret_sauce" )])
    def mission_5(self,username,password):
        self.waitForElementVisible((By.ID, 'user-name'))
        kullaniciAd=self.driver.find_element(By.ID, 'user-name')
        self.waitForElementVisible((By.ID, "password"))
        sifre=self.driver.find_element(By.ID, 'password')
        ...
        girisBtn.click()
        self.driver.save_screenshot(f"{self.folderPath}/mission_5.png")
    ...
    def mission_6(self):
        ...
        self.driver.save_screenshot(f"{self.folderPath}/mission_6.png")
        assert len(urunler) == 6
    
    @pytest.mark.parametrize("username,password",[("standard_user","secret_sauce")])
    def Mission_Add(self,username, password):
        kullaniciAd=self.driver.find_element(By.ID, "user-name")
        ...
        bpackAdd.click()
        self.driver.save_screenshot(f"{self.folderPath}/Mission_Add.png")

    
    @pytest.mark.parametrize("username, password", [("aaa", "000") , ("bbb", "111"), ("ccc", "222")])
    def Mission_InvalidLogin(self,username, password):
        self.waitForElementVisible((By.ID, "user-name"))
        ...
        self.driver.save_screenshot(f"{self.folderPath}/test_invalid_login_pass-{username}-{password}.png")
        assert Mesaj.text == "Epic sadface: Username and password do not match any user in this service"
    
    def Mission_Preis_To_Sort(self):
        self.waitForElementVisible((By.ID, "user-name"))
        kullaniciAd=self.driver.find_element(By.ID, "user-name")
        sifre=self.driver.find_element(By.ID, "password")
        girisBtn=self.driver.find_element(By.ID, "login-button")
        ...
        self.driver.save_screenshot(f"{self.folderPath}/Mission_Preis_To_Sort.png")
